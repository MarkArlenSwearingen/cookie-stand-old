...
function SimulatedNumberOfCookies(store){
  for (i = 0; i < HOURS.length; i++){
    var cookiesPerHour = Math.ceil(store.avgCookies) * (store.randomNumberOfCustomerPerHour());
    store.cookiesByHour.push(cookiesPerHour);
    totalCookiesByStore = totalCookiesByStore + cookiesPerHour;
    if(i === HOURS.length-1){
      store.cookiesByStore.push(totalCookiesByStore);
    }
  }
}
SimulatedNumberOfCookies(firstAndPike);
SimulatedNumberOfCookies(seaTac);
SimulatedNumberOfCookies(seattleCenter);
SimulatedNumberOfCookies(capitolHill);
SimulatedNumberOfCookies(alki);
